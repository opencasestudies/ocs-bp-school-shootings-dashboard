...
Shooting events can also have braod and lasting impacts for those who witness but are not directly involved. 
...
Thus, it is important that we characterize shootings so that we might better understand why shootings have occured to not only prevent the direct involvement of students in future events, but also to prevent students and staff from witnessing these events.
...
Now the data can be found at this [link](https://www.chds.us/ssdb/about/) and a csv file of the raw data can be dowloaded by clicking the "DOWNLOAD RAW DATA" botton.
...
We can see from this that many of the variables have Yes or No values, while others have relatively long distriptions. You may also notice that the `State` values are state abbreviations, not full state names. This is something that we will add to the data later. 
...
To do this, we will transform the coordinates using the `st_transform()` function  of the `sf` package into a two dimensional projection (called the [Albers equal-area conic projection](https://en.wikipedia.org/wiki/Albers_projection#:~:text=The%20Albers%20equal%2Darea%20conic,that%20uses%20two%20standard%20parallels.&text=The%20Albers%20projection%20is%20used,the%20United%20States%20Census%20Bureau.)) with units in meters using the [crs 102008](https://spatialreference.org/ref/esri/102008/html/) reference and then use the `st_jitter()` function of the `sf` package  to allow a specified amount of range near the actual original GPS coordinates. 
...
Notice how the  class for the `geometry` variable is now `<POINT [m]>` as our data has been transformed into corrdinates in meters.
...
We can now see that the coordiates are slighly modified. 
...
Our first plot could also have been made using `geom_bar()` instead of `geom_col()` this makes a similar plot but automatically uses the count for one of the axes, thus it is not required to first summarize the data using the `count()` function. 

#### {.think_question_block}
<b><u> Question Opportunity </u></b>

See if you can come up with the code for the plot using `geom_bar()` instead without using the `count()` function.
...
If after rounding we wanted zeros after the decimal so that the number of digits after the decimal was consistent for the different statistics we were dispalying, we could use the `format()` function to specify this. 
...
We also see a list on this page, `+` signs are used to indicate new items. Importantly two spaces are necesarry after each item to start a new line. 
