...
To implement the `case_when()` recoding of values, the existing values are written on the left of the `~` sign (quotation marks are necessary around the existing values) and new values are written on the right (quotations marks are not necessary as these are `TRUE` and `FALSE` statements). The `.default = NA` statement makes the handling of missing or unrecognized values explicit. We then use `count()` to check the resulting `TRUE`, `FALSE`, and `NA` values.
...
To do this, we will use the `across()` function from the `dplyr` package and the `matches()` function from the `tidyselect` package to apply this to all variables whose names match one of the patterns we want to change. Within `across()`, the `.` is a placeholder for the current column, so the same `case_when()` conditions are applied to each selected variable. 
