...
<style>
#TOC {
  background: url("img/icon-bahi.png");
...
shooting_data <- readr::read_csv(
  here::here("data", "wrangled", "shooting_data_wrangled_pre_map.csv"),
  show_col_types = FALSE
)

shooting_data_geocoded <- readr::read_csv(
  here::here("data", "wrangled", "shooting_data_wrangled_for_map.csv"),
  show_col_types = FALSE
)
...
We can quickly get a sense that the `dplyr` package is among the top most widely downloaded packages on CRAN.
...
```{r main-questions-check, echo=FALSE}
quiz(
  caption = "",
  question(
    "Which dashboard question connects most directly to the map?",
    answer("Where school shooting events occurred across the United States.", correct = TRUE),
    answer("Whether the original data were stored in an R package."),
    answer("Which R Markdown theme was selected."),
    allow_retry = TRUE,
...
```{r data-context-check, echo=FALSE}
quiz(
  caption = "",
  question(
    "Why does this case study repeatedly caution readers about interpretation?",
    answer("The data are compiled from open-source and third-party reporting and may include reporting errors.", correct = TRUE),
    answer("The dashboard uses no real data."),
    answer("The data are only about colleges and universities."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
Practice importing the local raw CSV file with `read_csv()` and `here()`.

####

```{r import-exercise-setup, include=FALSE}
library(here)
library(readr)
```

```{r import-exercise, exercise=TRUE, exercise.checker=gradethis::grade_learnr}
raw_school_shootings <- read_csv(
  ___("data", "raw", "K-12_SSDB_(Public)-K-12_SSDB_(Public)_Linked.csv")
)

nrow(raw_school_shootings)
```

```{r import-exercise-solution}
raw_school_shootings <- read_csv(
  here("data", "raw", "K-12_SSDB_(Public)-K-12_SSDB_(Public)_Linked.csv")
)

nrow(raw_school_shootings)
```

```{r import-exercise-check}
grade_code(
  pass_if(
    ~ exists("raw_school_shootings") && nrow(raw_school_shootings) > 0,
    "Nice. You imported the raw CSV from the project data folder."
  ),
  fail_if(~ TRUE, "Use `here()` to build the file path inside `read_csv()`.")
)
```
...
Thus we will use the `left_join(x,y)` function where `x` in this case will be the `shooting_data` (as it is introduced to this code first through the `%>%` pipe operator from the `dplyr` package) and `y` is the `state_df`. Thus, we add the `state_df` values where they match to the `shooting_data`.


We use the `%>%` pipe operator from the `dplyr` package and explicitly reassign the result back to the same object. We can therefore use `data_input <- data_input %>%` to pipe sequential steps and update `data_input` in place. We will demonstrate this in the code below.
...
By [piping](https://dplyr.tidyverse.org/articles/dplyr.html) we mean using the `%>%` pipe operator which is accessible after loading the `tidyverse` or the `dplyr` package directly. 
...
shooting_data %>%
       ...
       mutate(type = dplyr::case_when(`Suicide (or attempted suicide) by Shooter (Y/N)` == "Yes" ~ TRUE,
                                      `Suicide (or attempted suicide) by Shooter (Y/N)` == "No" ~ FALSE,
                                      `Suicide (or attempted suicide) by Shooter (Y/N)` == "Y" ~ TRUE,
                                      `Suicide (or attempted suicide) by Shooter (Y/N)` == "N" ~ FALSE,
                                      `Suicide (or attempted suicide) by Shooter (Y/N)` == "Officer Involved" ~ TRUE))
```
...
```{r}
shooting_data_wrangled_for_map <- shooting_data_wrangled_for_map %>%
...
Use `count()` to inspect the event categories in the wrangled dataset.

####

```{r wrangling-count-exercise-setup, include=FALSE}
library(dplyr)
shooting_data_wrangled_for_map <- readr::read_csv(
  here::here("data", "wrangled", "shooting_data_wrangled_for_map.csv"),
  show_col_types = FALSE
)
```

```{r wrangling-count-exercise, exercise=TRUE, exercise.checker=gradethis::grade_learnr}
shooting_data_wrangled_for_map %>%
  ___(Category, name = "events") %>%
  arrange(desc(events))
```

```{r wrangling-count-exercise-solution}
shooting_data_wrangled_for_map %>%
  count(Category, name = "events") %>%
  arrange(desc(events))
```

```{r wrangling-count-exercise-check}
grade_code(
  pass_if(
    ~ identical(.result, shooting_data_wrangled_for_map %>% count(Category, name = "events") %>% arrange(desc(events))),
    "Good work. This summary makes the most common event categories easy to compare."
  ),
  fail_if(~ TRUE, "Fill the blank with `count` and keep `Category` as the grouping variable.")
)
```
...
```{r}
deaths_perc_event <- deaths_perc_event %>%
...
Calculate yearly totals for shootings, deaths, and wounded people.

####

```{r yearly-summary-exercise-setup, include=FALSE}
library(dplyr)
shooting_data_wrangled_pre_map <- readr::read_csv(
  here::here("data", "wrangled", "shooting_data_wrangled_pre_map.csv"),
  show_col_types = FALSE
)
```

```{r yearly-summary-exercise, exercise=TRUE, exercise.checker=gradethis::grade_learnr}
yearly_summary <- shooting_data_wrangled_pre_map %>%
  group_by(Date_year) %>%
  summarise(
    Shootings = ___,
...
```{r yearly-summary-exercise-solution}
yearly_summary <- shooting_data_wrangled_pre_map %>%
  group_by(Date_year) %>%
  summarise(
    Shootings = n(),
...
```{r yearly-summary-exercise-check}
grade_code(
  pass_if(
    ~ exists("yearly_summary") && identical(yearly_summary, shooting_data_wrangled_pre_map %>% group_by(Date_year) %>% summarise(Shootings = n(), Deaths = sum(`Killed (includes shooter)`, na.rm = TRUE), Wounded = sum(Wounded, na.rm = TRUE), .groups = "drop")),
    "Exactly. `n()` counts incidents within each year, while `sum()` totals casualties."
  ),
  fail_if(~ TRUE, "Use `n()` for the number of shootings in each grouped year.")
)
```
...
```{r dashboard-basics-check, echo=FALSE}
quiz(
  caption = "",
  question(
    "What does `runtime: shiny` add to a flexdashboard?",
    answer("It allows dashboard outputs to respond to user inputs.", correct = TRUE),
    answer("It converts all plots into static image files."),
    answer("It removes the need for an R Markdown YAML header."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
per_year <- per_year %>%pivot_longer( cols = (-Date_year), 
                           values_to = "events", 
                           names_to = "id")
...
deaths_perc_event <- deaths_perc_event %>% bind_rows(greater_than4)
...
Notice that the `renderPlot()` function requires that the code be within brackets `{}`. The data is filtered first for just the state that was selected. The code for the plots is essentially the same with minor modifications to allow for all unique cases that the different states present. For example the `deaths_perc_event <- deaths_perc_event %>% filter(!duplicated(category))` is added to the last plot about the number of deaths per school shooting to avoid duplication of the rows in cases like Colorado where the there is only one event that had 4 or more deaths (because in the other cases this value is a sum of all school shooting with 4 or more deaths). 
...
leaflet(shooting_data_for_map) %>%
  ...
  addProviderTiles(provider = providers$Stadia.StamenTonerLite, group = "Toner Lite")%>%
  addCircleMarkers(popup = ~shooting_information0,
     ...
     group = "Circles") %>%
  addMiniMap(tiles = providers$Stadia.StamenToner,
              toggleDisplay = TRUE) %>%
  addLayersControl(
...
Create a compact table for the dashboard's data page.

####

```{r dashboard-table-exercise-setup, include=FALSE}
library(dplyr)
shooting_data_wrangled_pre_map <- readr::read_csv(
  here::here("data", "wrangled", "shooting_data_wrangled_pre_map.csv"),
  show_col_types = FALSE
...
```{r dashboard-table-exercise, exercise=TRUE, exercise.checker=gradethis::grade_learnr}
dashboard_table <- shooting_data_wrangled_pre_map %>%
  select(
    Date,
    School,
    City,
...
```{r dashboard-table-exercise-solution}
dashboard_table <- shooting_data_wrangled_pre_map %>%
  select(
    Date,
    School,
...
```{r dashboard-table-exercise-check}
grade_code(
  pass_if(
    ~ exists("dashboard_table") && "Wounded" %in% names(dashboard_table),
    "Nice. This smaller table keeps the dashboard focused on the fields users scan first."
  ),
  fail_if(~ TRUE, "Add `Wounded` as the final selected column.")
)
```
...
[Piping in R](https://dplyr.tidyverse.org/articles/dplyr.html){target="_blank"}   
