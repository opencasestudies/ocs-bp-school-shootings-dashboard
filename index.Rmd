...
Now the data can be found at this [link](https://www.chds.us/ssdb/about/) and a file of the raw data can be downloaded by clicking the "DOWNLOAD RAW DATA" button. This file was previously a `.CSV` file, but it is now an `.XLSX` file. To account for changes with this website, we have made the previous `.CSV` file available [here](https://github.com/opencasestudies/ocs-bp-school-shootings-dashboard/blob/master/raw/K-12_SSDB_(Public)-K-12_SSDB_(Public)_Linked.csv).
...
We will use the `here()` function of the `here` package to easily locate the data within the `raw` directory within the main directory that contains the `.Rproj` file.

```{r}
shooting_data <- 
  readr::read_csv(file = here::here("raw",
...
Now we will save the geocoded data in the `wrangled` directory using the `write_csv` function of the `readr` package.
