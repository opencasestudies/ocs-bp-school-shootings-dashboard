...
```{r}
valueBox(sum(pull(shooting_data, Wounded), na.rm = TRUE),
         color = "white")
```
...
```{r}
valueBox(sum(pull(shooting_data_geocoded,`Killed (includes shooter)`), na.rm = TRUE),
         color = "white")
```
...
valueBox(
  round(
  median(pull(shooting_data_geocoded,`Number of Shots Fired`),
      na.rm = TRUE),
  2),
  color = "white")
```
...
suicide <- sum(pull(shooting_data,`Suicide (Shooter was only victim) Y/N/ N/A`),
               na.rm = TRUE)/ sum(pull(shooting_data,`Killed (includes shooter)`), na.rm = TRUE)
suicide <- format(round(suicide*100, 1), nsmall = 1)
valueBox(paste0(suicide,"%"), color = "white")
```
...
valueBox(
  paste(
    ...
    round(
      100 * (sum(
    ifelse(pull(shooting_data,`Firearm Type`) == "Handgun",
                TRUE,
                FALSE), na.rm = TRUE)
    /
...
valueBox(
  paste(
    ...
    round(
      100 * (sum(
    ifelse(pull(shooting_data,`Shooter Gender`) == "Male",
                TRUE,
                FALSE), na.rm = TRUE)
    /
      length(shooting_data_geocoded$`Firearm Type`)),
...
selectInput("state_selected", label = "Select a state to explore:",
            choices = unique(shooting_data_geocoded %>%
                               filter(!is.na(State)) %>%
                               pull(State)), selected = "Maryland")

# There is no Washington, D.C. included above
```
...
renderPlot({
  ...
  shooting_data_geocoded_f2 <- shooting_data_geocoded[shooting_data_geocoded$State == input$state_selected, ]
