...
```{r}
shooting_per_year <- shooting_data_geocoded %>%
    group_by(Date_year) %>%
    count() %>%
    ungroup()
start <- 1970
...
```{r}
suicide <- sum(shooting_data_geocoded$`Suicide (or attempted suicide) by Shooter (Y/N)`,
               na.rm = TRUE) / length(shooting_data_geocoded$`Suicide (or attempted suicide) by Shooter (Y/N)`)
...
valueBox(
  paste(
    ...
    round(
      100 * (sum(
    ifelse(shooting_data_geocoded$`Firearm Type` == "Handgun",
                TRUE,
                FALSE), na.rm = TRUE)
    /
