...
```{r}
valueBox(value =
  paste(as.character(round(100 * (sum(pull(shooting_data,`Bullied (Y/N/ N/A)`), na.rm = TRUE)
    /
      shooting_data %>% filter(! is.na(`Bullied (Y/N/ N/A)` )) %>% pull(`Bullied (Y/N/ N/A)`) %>% length()),
    1)), "%"),
  color = "white",
  caption = paste0(as.character(round(shooting_data %>% filter(! is.na(`Bullied (Y/N/ N/A)` )) %>% pull(`Bullied (Y/N/ N/A)`) %>% length()/ shooting_data %>%pull(`Bullied (Y/N/ N/A)`) %>% length() *100), 1), "% reporting"))
```

### **Shooter experienced domestic violence**

```{r}
valueBox(value =
  paste(as.character(round(100 * (sum(pull(shooting_data,`Domestic Violence (Y/N)`),
                                      na.rm = TRUE)
    /
     shooting_data %>% filter(! is.na(`Domestic Violence (Y/N)` )) %>% pull(`Domestic Violence (Y/N)`) %>% length()),
    1)), "%"),
  color = "white")

```

### **Shooter planned attack**

```{r}
valueBox(value =
  paste(as.character(round(100 * (sum(pull(shooting_data,`Pre-planned school attack`),
                                      na.rm = TRUE)
    /
      shooting_data %>% filter(! is.na(`Pre-planned school attack` )) %>% pull(`Pre-planned school attack`) %>% length()),
    1)), "%"),
  color = "white")
...
valueBox(value =
  paste(as.character(round(100 * (sum(
    case_when(pull(shooting_data,`Shooter Gender`) == "Male" ~ TRUE,
                                      ...
                                      na.rm = TRUE)
    /
      length(pull(shooting_data, `Shooter Gender`))),
    1)), "%"),
  color = "white")
```
...
When concerned about seeing troubling behaviors, tell a trusted adult.
