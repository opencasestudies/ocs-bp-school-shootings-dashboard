...
```{r}
shooting_data <- read_csv(here("data", "wrangled",
                                        "shooting_data_wrangled_pre_map.csv"))

shooting_data_geocoded <- read_csv(here("data", "wrangled",
...
per_year_cum <- per_year_cum %>%pivot_longer( cols = c(Shootings,
                                        Deaths ), 
                           values_to = "events", 
                           names_to = "id")
...
deaths_perc_event <- deaths_perc_event %>% mutate(category = paste0(`Killed (includes shooter)`, " deaths ", "\n(", percent, "%)")) 
