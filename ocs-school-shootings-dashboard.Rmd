...
To calculate the percentage of shootings where this infromation was reported we can do the following, by calculating all values that are not `NA` using `>=0` and calculating the number all possible values using the base `length()` function.
...
avocado if we add `###` forthe source like usual, then we will no longer have the subsequent headers following the <details> section
...
<style>
div.black { background-color:#8FBC8F; border-radius: 5px; padding: 20px; font-size: 1em;color: white;}
</style>
<div class = "black">
Riedman, David, and Desmond O’Neill. “CHDS – K-12 School Shooting Database.” Center for Homeland Defense and Security, June 2020, [www.chds.us/ssdb](www.chds.us/ssdb).
</div>
...
<style>
div.blue { background-color:"black"; border-radius: 5px; padding: 20px; font-size: .8em;}
</style>
<div class = "blue">
...
To calculate the percentage of shootings where the shooter committed or attempted suicide, we will use our calculation which was explained in the [Data Analsysis and Visualization] section. The `paste0` function is used to add the percentage symbol.
...
Additional text about the reporting rate for these statisitcs was added using the `###` syntax. Additionally inline code is evaluated using the notation `"`r `"` Again notice that `"'"` was used instead of `"`"` just for illustrative purposes to allow this R Markdown document to render the code from the dashboard file.
...
### Details
 ...
 <details> <summary> Click here to see the code for this pgge. </summary>
...
In this case study we have demonstrated the basics of R Markdown and how to create a dashboard with using the `flexdashboard` package. We also demonstrated how to include an interactive table with the `DT` package, how to include interactive plots using functions of the `shiny` package such as `renderPlot()`. We also included interactive value boxes using the `renderValueBox()` function of the `flexdashboard` package wich works with the `shiny` package. Finally we also showed how to include interactive maps using the `leaflet` package. 

This case study also explored how to properly caculate and interpret percentages when the data has missing values. We also discussed the benefits and limiting aspects of pie charts (using `ggplot2`) and waffle plots (using `waffle`).

Overall the dashboard that we created shows that the number of shootings per year has increased overtime. Further investigation is necessary to determine if this is simply due to increases in population alone or if the rate has increased due to other factors and if so, what those factors might be. It is also clear that the number of shootings and the number of deaths percapita varies by state. Thus there appears to be other aspects accounting for state differences. 
