...
We see that many of the varaibles that are of class character, which is indicated by the `<chr>` below the variable name have either `Yes` or `No` values or `Y` and `N` values. We can make these consistently `TRUE` and `FALSE` by using the `recode()` function of the `dplyr` package. This function allows us to specify new values for existing values. The exisiting values are written on the left of the `=` sign (no quotation marks necessary) and new values are written on the right (quotations marks are necessary as these are new values). We will also use the `across()` function of the `dplyr` package and the `where()` function of the `tidyselect` packge to allow us to apply this to all of the variables that are of class character. This will be specified using the base `is.character()` function. The columns that are of class character are selected using the `where()` function and the `across()` function then applies the recode function to all of these variables. Notice that the `~` symbol is necessary before the funtion that is applied using `across()`.
