...
[Here](https://beta.rstudioconnect.com/jjallaire/htmlwidgets-highcharter/) you can see an example of a dashboard created in R.


```{r}
knitr::include_url("https://beta.rstudioconnect.com/jjallaire/htmlwidgets-highcharter/")
```
...
Now we will combine this with our data using the `left_join()` function of the `dplyr` package. avocado add explanation and join images
...
<details><summary> click here for more details about the differences between `recode()` and case_when()`</summary>
...
Notice how when using summarize we dont use mutate here.
```{r}
...
shootings_per_year_cum <- shootings_per_year%>%
    mutate("Shootings_cumsum" = cumsum(Shootings))
...
per_year_cum %<>%pivot_longer( cols = c(Shootings_cumsum,
                                        Deaths_cumsum ), 
                           values_to = "events", 
                           names_to = "id")
...
shootings_per_year_cum %>%
    ggplot(aes(x = Date_year, y = Shootings_cumsum)) +
    geom_col(fill = "black") +
    scale_x_continuous(breaks = seq(start, end, by = 5),
                 labels = seq(start, end, by = 5),
                 limits = c(start-1, end+1)) +
    theme_minimal() +
  labs(title = "Yearly Cumulative Number of School Shootings",
...
shooting_data %>%
  ...
  #                    labels = seq(0,10, by = 1),
  #                    limits = c(-1,10)) +
ggforce::facet_zoom(xlim = c(3, max(pull(shooting_data, `Killed (includes shooter)`))), ylim = c(0,30))+
theme_minimal() +
    labs(title = "Deaths per School Shooting",
         subtitle = "United States",
...
deaths_perc_event <-shooting_data %>%
  ...
  rename("num_events"= n) %>%
   mutate(percent = (num_events/sum(num_events)*100))

greater_than4<-deaths_perc_event %>% filter(`Killed (includes shooter)` >=4) %>% colSums()
greater_than4

deaths_perc_event %<>%  bind_rows(greater_than4) %>% mutate(category = paste(`Killed (includes shooter)`, "deaths")) 

last(pull(deaths_perc_event, category))

# deaths_perc_event %<>% mutate(category= recode(category, "85 deaths" = "4+deaths"))

deaths_perc_event %<>% mutate(category= case_when(category ==  last(pull(deaths_perc_event, category)) ~ "4+deaths", TRUE ~category ))
...
deaths_perc_event %>% select(-`Killed (includes shooter)`) %>%
  ...
  waffle( 
          legend_pos = "bottom") +  scale_fill_viridis_d()

#colors = c("grey54", "black", "red","grey77", "#009bda" ),
deaths_perc_event %>%
  filter(percent>0.5) %>%
    ggplot(aes(x = "", y = percent, fill = category)) +
    geom_bar(stat = "identity", color = "black") +
    coord_polar("y", start = 0) + scale_y_continuous(breaks= NULL)+
  scale_fill_viridis_d()
...
OK, now we are ready to make our dashboard!


### Dashboard packages
To make our dashboard we will use 3 very useful packages:
...
The case study that you are reading right now was created using an [R markdown document](https://rmarkdown.rstudio.com/). This means that it is a document that uses the `Markdown` language. 
...
[R Markdown](https://rmarkdown.rstudio.com/articles_intro.html#:~:text=R%20Markdown%20is%20a%20file,code%2C%20like%20the%20document%20below.) is a file format that contains markdown sytax and embedded R code (it can also incoperate code from some other languages like [Python](https://en.wikipedia.org/wiki/Python_(programming_language)) and [SQL](https://en.wikipedia.org/wiki/SQL)).
...
In our case we are interested in rendering our markdown document into a website. Thus the code in our R markdown document will be interpreted and converted to html code.
...
This metadata specifies the layout and elements of the dashboard.





Components within the dashboard must be delinated by using `###` - if you are familiar with markdown notation, this is a level 3 markdown header.




By default dashboards will have a column based layout.



### Columns & Rows

In the metadata, we specify whether we want our output to be oriented in rows (left to right) or columns (top to bottom). 

If we create two plots with columnwise settings, the first plot will be at the top and the second at the bottom.

If we create two plots with rowwise settigns, the first plot will be on the left and the second plot will be on the right. 

Each time we specify a new column or row, the output is shown in this way in a specific column/row. 

See `Multiple Columns` and `Row Orientation` on the `RStudio/flexdashboard/Home` link included above for an example. 

To specify a column or row we can use a dashed line like so:
...
To add tabsets to columns/rows we can use the following: 
