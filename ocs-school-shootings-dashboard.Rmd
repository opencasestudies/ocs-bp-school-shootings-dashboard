...
different <-setdiff(shooting_data[1:48], shooting_data_geocoded[1:48])

diff_narrative<-setdiff(shooting_data$`Narrative (Detailed Summary/ Background)`, shooting_data_geocoded$`Narrative (Detailed Summary/ Background)`)
...
Now we will add the full state name to the data using data related to the US 50 states in a dataset called `state` that is automatically loaded with R sessions in the `datasets` package. The `state.abb` object is a list of the state abbreviations and `state.name` is a list of the state names.
...
```{r}
state_df <- tibble(state.abb, state.name)
...
shooting_data_geocoded %>% select(Date, Date_year)
```
Looks good!
...
shooting_data_geocoded %>% filter(`Targeted Specific Victim(s)` == "No victoms")

shooting_data_geocoded %>% select(matches("Y/N|Specific|Random|Pre-planned"))
...
```{r, eval = FALSE}
shooting_data_geocoded %>%
      mutate(dplyr::across(.cols = where(is.character), ~ dplyr::recode(.,
                      Yes = "TRUE",
                       No = "FALSE",
                        Y = "TRUE",
                        N = "TRUE"))) %>%
 mutate(dplyr::across(.cols = where(is.character), ~ as.logical(.)))


shooting_data_geocoded %>%
       mutate(dplyr::across(.cols = where(is.character) & n_distinct(.) <=2, ~ (dplyr::case_when(. == "Yes" ~ TRUE,
                       .== "No" ~ FALSE,
                        .== "Y" ~ TRUE,
                       .== "N" ~ TRUE))))

#where(is.character & n_distinct(.)<2)

shooting_data_geocoded %>%
       mutate(dplyr::across(.cols = where(is.character) & contains("Y/N|Specific|Random"), ~ (dplyr::case_when(. == "Yes" ~ TRUE))))
...
shooting_per_year<- shooting_data %>%
       mutate(Date = lubridate::mdy(Date)) %>%
  mutate(Date_year = lubridate::year(Date)) %>%
    group_by(Date_year) %>%
    count() %>%
...
shooting_per_year_cum <- shooting_per_year%>%
    mutate(n_cumsum = cumsum(n))

shooting_per_year_cum 
...
shooting_data %>%
  ...
  #                    labels = seq(0,10, by = 1),
  #                    limits = c(-1,10)) +
ggforce::facet_zoom(xlim = c(10, max(pull(shooting_data, `Killed (includes shooter)`))))+
theme_minimal() +
    labs(title = "Deaths per School Shooting",
         subtitle = "United States",
         x = "School Shootings",
...
deaths_perc_event %>% select(-`Killed (includes shooter)`) %>%
  ...
  waffle( colors = c("grey54", "black", "red","grey77", "#009bda" ),
          legend_pos = "bottom")
...
```{r}
DT::datatable(shooting_data_geocoded)
```
