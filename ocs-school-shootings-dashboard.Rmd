...
```{r}
deaths_perc_event %<>% mutate(category = paste(`Killed (includes shooter)`, "deaths")) 
...
deaths_perc_event %<>% 
  mutate(category = case_when(
    category ==  last(pull(deaths_perc_event, category)) ~ "4+ deaths",
...
deaths_perc_event %>% select(-`Killed (includes shooter)`) %>%
  filter(category %in% c("0 deaths", "1 death", "2 deaths", "3 deaths", "4+ deaths")) %>%
  mutate(percent = round(percent)) %>%
  select(-num_events) %>%
  tidyr::pivot_wider(names_from = category, 
                    values_from = percent) %>%
  waffle::waffle(legend_pos = "bottom") +  scale_fill_viridis_d()
...
2) Components within the dashboard are delinated by using `###` - if you are familiar with markdown notation, this is a level 3 markdown header.

This can include text or not


3) To  include a plot or any output from R, use the following syntax:
...
4) To add columns or rows the following syntax is used `---------`. By default this notation will create new columns, however if the YAML is modifed to specify to create rows, than this same sytnax will be used to create rows. The number of dashes does not matter.

5) To add a page to a navigation bar (also called a navbar) the following syntax is used `=======`. The number of dashes does not matter.
...
```{r}
knitr::include_graphics(here::here("img", "rmdtype.png"))
...
We also need to load the necessary packages and the wrangled data that we created in this Rmd file.
...
To create the structure for a page that will display the data we will will have two columns, with the first one (on the left) wider than the other:
...
Then we will add our `DT_table` to the first column. First we need to include the code that we previously used to create `DT_table` in this R markdown document :
...
In our dashboard we will also use this syntax `=====` to designate items on the navigation bar.
