...
4) To add columns or rows the following syntax is used `---------`. By default this notation will create new columns, however if the YAML is modifed to specify to create rows, than this same sytnax will be used to create rows. The number of dashes does not matter. (This is a level 2 header in markdown, just like `##`)

5) To add a page to a navigation bar (also called a navbar) the following syntax is used `=======`. The number of dashes does not matter. (This is a level 1 header in markdown, just like `#`)
...
To create a value box we will use the `valueBox()` function of the `flexdashboard` package.
```
### **Wounded**
...
### Sate Statistics Page.

Now we will create the next page, the **State Statistics**  page. On this page we want to be able to select data for a specific state and render plots and get statistics just for the selected state. To do this we will utalize the `renderPlot()` and `renderValueBox()` functions of the `flexdashboard` package, as well as the `selectInput()` function of the `shiny` package.
